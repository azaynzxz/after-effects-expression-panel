...
function createXBoltCropUI(thisObj) {
    ...
    nameInput.helpTip = "Name for the precomp (used by Xpert Crop). Leave empty for default.";
...
*/
function processBoltCrop(precompFirst, precompName) {
    ...
    if (precompFirst) {
        // Calculate the time span of selected layers before precomposing
        var earliestIn = Infinity;
        var latestOut = -Infinity;
        var indices = [];
        for (var i = 0; i < selectedLayers.length; i++) {
            indices.push(selectedLayers[i].index);
            if (selectedLayers[i].inPoint < earliestIn) earliestIn = selectedLayers[i].inPoint;
            if (selectedLayers[i].outPoint > latestOut) latestOut = selectedLayers[i].outPoint;
        }
        var compName = (precompName && precompName.length > 0) ? precompName : "Precomp";
        ...
        // Adjust precomp duration to match the time span of the selected layers
        var layerSpan = latestOut - earliestIn;
        if (layerSpan > 0) {
            newComp.duration = layerSpan;
            newComp.displayStartTime = 0;

            // Shift internal layers so they start relative to time 0
            for (var s = 1; s <= newComp.layers.length; s++) {
                var innerLayer = newComp.layer(s);
                if (innerLayer && !innerLayer.locked) {
                    try {
                        innerLayer.startTime = innerLayer.startTime - earliestIn;
                    } catch (e) {
                        // Skip locked or otherwise immovable layers
                    }
                }
            }
        }
...
// Function to show X Crop as a non-modal palette
function showXCropDialog() {
    var dialog = new Window("palette", "X Crop", undefined, { resizeable: true });
