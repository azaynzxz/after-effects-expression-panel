...
var EXPRESSIONS = {
    ...
    "LR": "amp = 50;\nframesPerCycle = 5;\nfps = thisComp.frameDuration;\nt = time / (framesPerCycle * fps);\nvalue + [Math.sin(t * 2 * Math.PI) * amp, 0];",
    "RPP": "let amp = 2;\nlet rotateFreq = 8;\n\n// Get a stable cycle index based on time\nlet cycleIndex = Math.floor(time);\n\n// Generate a pseudo-random value per cycle (0...1)\nfunction random(seed) {\n  return fract(Math.sin(seed * 91.345) * 47453.321);\n}\nfunction fract(x) {\n  return x - Math.floor(x);\n}\n\n// Get a random pause duration between 0.3 and 0.8 seconds\nlet rand = random(cycleIndex);\nlet pauseDuration = 0.3 + rand * (0.8 - 0.3);\n\n// Total cycle time: rotateDuration + pauseDuration\nlet rotateDuration = 0.5; // how long it rotates each cycle\nlet cycleTime = rotateDuration + pauseDuration;\n\n// Where we are in the current cycle\nlet t = time % cycleTime;\n\nlet output = (t < rotateDuration)\n  ? Math.sin(time * rotateFreq * 2 * Math.PI) * amp\n  : 0;\n\noutput;",
    "TF": "seedRandom(index + Math.floor(time), true);\n\nrand = random();  // Random chance per second\nrate = rand > 0.7 ? 20 : rand > 0.4 ? 8 : 2;\n\nt = time * rate;\nflicker = Math.floor(t) % 2 == 0 ? 100 : 0;\n\nflicker",
    "HL": "// Seed randomness per cycle\ncycleIndex = Math.floor(time);\nseedRandom(index + cycleIndex, true);\n\n// Random pause duration (0.3s–1.0s)\npauseDuration = 0.3 + random() * 0.7;\nflickerDuration = 0.4; // duration of flicker before pausing\ncycleTime = pauseDuration + flickerDuration;\n\nt = time % cycleTime;\n\n// If within flicker time, do horror flicker; otherwise, stay dim\nif (t < flickerDuration) {\n  // Seed for flicker randomness\n  seedRandom(index + Math.floor(time * 10), true);\n\n  rate = 10 + random() * 40;\n  flick = (Math.sin(time * rate * 6.2831) * 43758.5453) % 1;\n  flick = flick - Math.floor(flick); // same as fract()\n\n  // Flicker behavior\n  flickerValue = flick < 0.2 ? 100 :\n                 flick < 0.25 ? 70 :\n                 flick < 0.3 ? 40 : \n                 10 + flick * 20; // low-level jitter\n} else {\n  flickerValue = 0; // paused (off)\n}\n\nflickerValue;",
    "SP": "// Settings\nminScale = 90;\nmaxScale = 110;\nspeed = 2; // cycles per second\n\n// Oscillate value using sine\ns = (Math.sin(time * Math.PI * speed) + 1) / 2; // normalized between 0–1\n\n// Interpolate scale using linear easing\nscaleVal = linear(s, 0, 1, minScale, maxScale);\n[scaleVal, scaleVal]",
    "LC": 'loopOut("cycle")',
    "LCN": 'loopOut("offset")',
    "LPP": 'loopOut("pingpong")',
    "WF": "wiggle(1,50)",
    "GL": "offset = 0.5;\nframesPerToggle = 12;\nflicker = Math.floor(timeToFrames(time - offset)) % (framesPerToggle * 2) < framesPerToggle ? 100 : 0;\nflicker"
};

// Mapping from abbreviations to full names for dialog functions
var ABBREV_TO_FULL = {
    "FW": "Fast Wiggle",
    "PT": "Posterize Time",
    "SM": "Stop Motion",
...
function createPanel(thisObj) {
    ...
    myPanel.preferredSize.width = 130; // Ultra-compact width for abbreviations
    ...
    title.graphics.font = ScriptUI.newFont("Arial", "BOLD", 10); // Smaller font for compact width
    ...
    timeInput.preferredSize.width = 60; // Ultra-compact width
    ...
    calibrationInput.preferredSize.width = 25; // Reduced width
    ...
    // Navigation buttons grid: 3 columns x 2 rows (compact layout)
    var buttonsGrid = timelineGroup.add("group");
    buttonsGrid.orientation = "column";
    buttonsGrid.alignChildren = ["fill", "top"];
    buttonsGrid.spacing = 2;

    // Row 1: Backward buttons (3 columns)
    var row1 = buttonsGrid.add("group");
    row1.orientation = "row";
    row1.alignChildren = ["fill", "center"];
    row1.spacing = 2;

    var jumpBack1s = row1.add("button", undefined, "-1s");
    jumpBack1s.preferredSize.width = 38; // Ultra-tight width
    jumpBack1s.preferredSize.height = 20; // Reduced height
    jumpBack1s.helpTip = "Jump backward 1 second";
    jumpBack1s.onClick = function() {
        jumpByTime(-1);
    };

    var jumpBack05s = row1.add("button", undefined, "-0.5s");
    jumpBack05s.preferredSize.width = 38; // Ultra-tight width
    jumpBack05s.preferredSize.height = 20; // Reduced height
    jumpBack05s.helpTip = "Jump backward 0.5 seconds";
    jumpBack05s.onClick = function() {
        jumpByTime(-0.5);
    };

    var jumpBack4f = row1.add("button", undefined, "-4f");
    jumpBack4f.preferredSize.width = 30; // Extra tight for frame buttons
    jumpBack4f.preferredSize.height = 18; // Reduced height
    jumpBack4f.helpTip = "Jump backward 4 frames";
    jumpBack4f.onClick = function() {
        jumpByFrames(-4);
    };

    // Row 2: Frame input, Go button, and forward buttons (3 columns)
    var row2 = buttonsGrid.add("group");
    row2.orientation = "row";
    row2.alignChildren = ["fill", "center"];
    row2.spacing = 2;

    var frameInput = row2.add("edittext", undefined, "");
    frameInput.preferredSize.width = 38; // Ultra-tight width
    frameInput.preferredSize.height = 20; // Reduced height
    frameInput.helpTip = "Enter number of frames to jump";

    var frameJumpBtn = row2.add("button", undefined, "Go");
    frameJumpBtn.preferredSize.width = 30; // Tight width
    frameJumpBtn.preferredSize.height = 20; // Reduced height
    frameJumpBtn.helpTip = "Jump by entered frames";
    frameJumpBtn.onClick = function() {
        jumpByFrames(parseInt(frameInput.text));
    };

    var jumpForward4f = row2.add("button", undefined, "+4f");
    jumpForward4f.preferredSize.width = 30; // Extra tight for frame buttons
    jumpForward4f.preferredSize.height = 18; // Reduced height
    jumpForward4f.helpTip = "Jump forward 4 frames";
    jumpForward4f.onClick = function() {
        jumpByFrames(4);
    };

    // Row 3: Remaining forward buttons (2 columns, will span)
    var row3 = buttonsGrid.add("group");
    row3.orientation = "row";
    row3.alignChildren = ["fill", "center"];
    row3.spacing = 2;

    var jumpForward05s = row3.add("button", undefined, "+0.5s");
    jumpForward05s.preferredSize.width = 38; // Ultra-tight width
    jumpForward05s.preferredSize.height = 20; // Reduced height
    jumpForward05s.helpTip = "Jump forward 0.5 seconds";
    jumpForward05s.onClick = function() {
        jumpByTime(0.5);
    };

    var jumpForward1s = row3.add("button", undefined, "+1s");
    jumpForward1s.preferredSize.width = 38; // Ultra-tight width
    jumpForward1s.preferredSize.height = 20; // Reduced height
    jumpForward1s.helpTip = "Jump forward 1 second";
    jumpForward1s.onClick = function() {
        jumpByTime(1);
    };

    // Empty space filler for row 3 to maintain 3-column alignment
    var spacer = row3.add("statictext", undefined, "");
    spacer.preferredSize.width = 38;

    // Calibration +/- buttons row at bottom
    var calButtonsRow = timelineGroup.add("group");
    calButtonsRow.orientation = "row";
    calButtonsRow.alignChildren = ["left", "center"];
    calButtonsRow.spacing = 2;
    
    var calLabelBottom = calButtonsRow.add("statictext", undefined, "Cal:");
    calLabelBottom.graphics.font = ScriptUI.newFont("Arial", "BOLD", 9);
    
    var calMinusBtn = calButtonsRow.add("button", undefined, "-");
    calMinusBtn.preferredSize.width = 20; // Compact width
    calMinusBtn.preferredSize.height = 20; // Compact height
    calMinusBtn.helpTip = "Decrease calibration by 50ms";
    calMinusBtn.onClick = function() {
        var current = parseFloat(calibrationInput.text) || 0;
        calibrationInput.text = (current - 50).toString();
    };
    
    var calPlusBtn = calButtonsRow.add("button", undefined, "+");
    calPlusBtn.preferredSize.width = 20; // Compact width
    calPlusBtn.preferredSize.height = 20; // Compact height
    calPlusBtn.helpTip = "Increase calibration by 50ms";
    ...
    // Add Current Keyframes button (abbreviated)
    var addKeyframesBtn = utilRow1.add("button", undefined, "AK");
    addKeyframesBtn.preferredSize.width = 38; // Tight width
    addKeyframesBtn.preferredSize.height = 15; // Reduced height
    addKeyframesBtn.helpTip = "Add Keyframes: Adds keyframes for current position, scale and rotation values";
    addKeyframesBtn.onClick = function() {
        addCurrentKeyframes();
    };
    
    // Add Hide Layers button (abbreviated)
    var hideLayersBtn = utilRow1.add("button", undefined, "HL");
    hideLayersBtn.preferredSize.width = 38; // Tight width
    hideLayersBtn.preferredSize.height = 15; // Reduced height
    hideLayersBtn.helpTip = "Hide Layers: Hide all layers starting with 'hide' or 'x' in main_comp";
    hideLayersBtn.onClick = function() {
        hideAllLayersNamedHide();
    };
    
    // Add Show Layers button (abbreviated)
    var showLayersBtn = utilRow1.add("button", undefined, "SL");
    showLayersBtn.preferredSize.width = 38; // Tight width
    showLayersBtn.preferredSize.height = 15; // Reduced height
    showLayersBtn.helpTip = "Show Layers: Show all layers starting with 'hide' or 'x' in main_comp";
    ...
    // Add Trim Selected button (abbreviated)
    var trimSelectedBtn = utilRow2.add("button", undefined, "TS");
    trimSelectedBtn.preferredSize.width = 38; // Tight width
    trimSelectedBtn.preferredSize.height = 15; // Reduced height
    trimSelectedBtn.helpTip = "Trim Selected: Trim selected layers to avoid overlapping";
    trimSelectedBtn.onClick = function() {
        trimSelectedLayers();
    };
    
    // Add Batch Duration button (abbreviated)
    var batchDurationBtn = utilRow2.add("button", undefined, "BD");
    batchDurationBtn.preferredSize.width = 38; // Tight width
    batchDurationBtn.preferredSize.height = 15; // Reduced height
    batchDurationBtn.helpTip = "Batch Duration: Change duration of selected precomp source compositions";
    batchDurationBtn.onClick = function() {
        changeBatchDuration();
    };
    
    // Add Separate Jump Window button (abbreviated)
    var separateJumpBtn = utilRow2.add("button", undefined, "JW");
    separateJumpBtn.preferredSize.width = 38; // Tight width
    separateJumpBtn.preferredSize.height = 15; // Reduced height
    separateJumpBtn.helpTip = "Jump Window: Open timeline jump controls in a separate window";
    ...
    mainGroup.spacing = 1; // Ultra-tight spacing
    ...
    leftCol.preferredSize.width = 60; // Ultra-compact width for abbreviations
    ...
    // Basic animation buttons (abbreviated)
    var basicButtons = ["FW", "PT", "SM", "TR", "UD", "LR", "RPP", "TF", "HL", "SP"];
    ...
    // Loop buttons (abbreviated)
    var loopButtons = ["LC", "LCN", "LPP"];
    ...
    // Scale & Center button (abbreviated)
    var scaleAndCenterBtn = scaleButtonRow.add("button", undefined, "S&C");
    scaleAndCenterBtn.alignment = ["fill", "center"];
    scaleAndCenterBtn.preferredSize.width = 45; // Tight width
    scaleAndCenterBtn.preferredSize.height = 15; // Reduced height
    scaleAndCenterBtn.helpTip = "Scale & Center: Zoom selected layer based on shape layer area (like a crop/zoom window)";
    
    // Refresh button
    var refreshBtn = scaleButtonRow.add("button", undefined, "↻");
    refreshBtn.preferredSize.width = 18; // Tight width
    refreshBtn.preferredSize.height = 15; // Reduced height
    ...
    rightCol.preferredSize.width = 60; // Ultra-compact width for abbreviations
    ...
    // Complex animation buttons (abbreviated)
    var complexButtons = ["WF", "GL"];
    ...
    xLockBtn.preferredSize.width = 50; // Tight width
    xLockBtn.preferredSize.height = 15; // Reduced height
    xLockBtn.helpTip = "Toggle lock status for layers named 'x' or 'X' in main_comp";
    xLockBtn.onClick = function() {
        toggleXLockLayers();
    };
    
    // Add Auto Trim button (abbreviated)
    var autoTrimBtn = utilityGroup.add("button", undefined, "AT");
    autoTrimBtn.preferredSize.width = 50; // Tight width
    autoTrimBtn.preferredSize.height = 15; // Reduced height
    autoTrimBtn.helpTip = "Auto Trim: Trim overlapping layers automatically in main_comp";
    autoTrimBtn.onClick = function() {
        autoTrimLayers();
    };
    
    // Add Copy Audio button (abbreviated)
    var copyAudioBtn = utilityGroup.add("button", undefined, "CA");
    copyAudioBtn.preferredSize.width = 50; // Tight width
    copyAudioBtn.preferredSize.height = 15; // Reduced height
    copyAudioBtn.helpTip = "Copy Audio: Copy audio comp from main_comp to current comp and sync it";
    ...
    choppyFlipBtn.preferredSize.width = 50; // Tight width
    choppyFlipBtn.preferredSize.height = 15; // Reduced height
    choppyFlipBtn.helpTip = "Choppy Flip: Add choppy left-right flip animation to selected layers";
    choppyFlipBtn.onClick = function() {
        showChoppyFlipDialog();
    };
    
    // Add Water Distort button (abbreviated)
    var waterDistortBtn = utilityGroup.add("button", undefined, "WD");
    waterDistortBtn.preferredSize.width = 50; // Tight width
    waterDistortBtn.preferredSize.height = 15; // Reduced height
    waterDistortBtn.helpTip = "Water Distort: Adds water distortion effect to selected layers";
    waterDistortBtn.onClick = function() {
        showWaterDistortionDialog();
    };
    
    // Add Bounce x2 button (abbreviated)
    var bounceX2Btn = utilityGroup.add("button", undefined, "B2");
    bounceX2Btn.preferredSize.width = 50; // Tight width
    bounceX2Btn.preferredSize.height = 15; // Reduced height
    bounceX2Btn.helpTip = "Bounce x2: Creates 2 bouncy keyframes: 0→100 (scale only)";
    bounceX2Btn.onClick = function() {
        addBounceKeyframes();
    };
    
    // Add Puppet→Null button (abbreviated)
    var puppetToNullBtn = utilityGroup.add("button", undefined, "P→N");
    puppetToNullBtn.preferredSize.width = 50; // Tight width
    puppetToNullBtn.preferredSize.height = 15; // Reduced height
    puppetToNullBtn.helpTip = "Puppet→Null: Create null objects for puppet pins on selected layer(s) - supports multiple layers";
    puppetToNullBtn.onClick = function() {
        createPuppetNulls();
    };
    
    // Add Flip H button (abbreviated)
    var flipHorizontalBtn = utilityGroup.add("button", undefined, "FH");
    flipHorizontalBtn.preferredSize.width = 50; // Tight width
    flipHorizontalBtn.preferredSize.height = 15; // Reduced height
    flipHorizontalBtn.helpTip = "Flip H: Flip layers horizontally (invert X scale)";
    flipHorizontalBtn.onClick = function() {
        flipHorizontal();
    };
    
    // Add Flip V button (abbreviated)
    var flipVerticalBtn = utilityGroup.add("button", undefined, "FV");
    flipVerticalBtn.preferredSize.width = 50; // Tight width
    flipVerticalBtn.preferredSize.height = 15; // Reduced height
    flipVerticalBtn.helpTip = "Flip V: Flip layers vertically (invert Y scale)";
    flipVerticalBtn.onClick = function() {
        flipVertical();
    };
    
    // Add Reverse KF button (abbreviated)
    var reverseKeyframesBtn = utilityGroup.add("button", undefined, "RK");
    reverseKeyframesBtn.preferredSize.width = 50; // Tight width
    reverseKeyframesBtn.preferredSize.height = 15; // Reduced height
    reverseKeyframesBtn.helpTip = "Reverse KF: Reverse selected keyframes (select keyframes in timeline first)";
    reverseKeyframesBtn.onClick = function() {
        reverseAllKeyframes();
    };
    
    // Add Batch Scale button (abbreviated)
    var batchScaleBtn = utilityGroup.add("button", undefined, "BS");
    batchScaleBtn.preferredSize.width = 50; // Tight width
    batchScaleBtn.preferredSize.height = 15; // Reduced height
    batchScaleBtn.helpTip = "Batch Scale: Batch scale layers with presets (34%, 50%, 100%)";
    ...
    advancedToolsBtn.preferredSize.width = 50; // Tight width
    advancedToolsBtn.preferredSize.height = 15; // Reduced height
    ...
    prevLayerBtn.preferredSize.width = 28; // Tight width
    prevLayerBtn.preferredSize.height = 18; // Reduced height
    ...
    nextLayerBtn.preferredSize.width = 28; // Tight width
    nextLayerBtn.preferredSize.height = 18; // Reduced height
    ...
    function addButtonsToGroup(group, buttonNames) {
        for (var i = 0; i < buttonNames.length; i++) {
            ...
            var fullName = ABBREV_TO_FULL[key] || key; // Get full name if abbreviation exists
            if (EXPRESSIONS.hasOwnProperty(key) || key === "Fish-like") {
                var btn = group.add("button", undefined, key);
                btn.alignment = "fill";
                btn.preferredSize.width = 50; // Tight width for abbreviations
                btn.preferredSize.height = 15; // Reduced height
                // Show original name in tooltip
                var tooltipText = fullName !== key ? fullName + ": " + (EXPRESSIONS[fullName] || EXPRESSIONS[key]) : (key === "Fish-like" ? "Swimming fish animation" : (EXPRESSIONS[fullName] || EXPRESSIONS[key]));
                btn.helpTip = tooltipText;
                
                // Create closure for button click
                (function(expressionName, expressionCode, fullExpressionName) {
                    btn.onClick = function() {
                        // Use full name for dialog functions
                        if (fullExpressionName === "Time Rotation") {
                            showTimeRotationDialog();
                        } else if (fullExpressionName === "Up Down") {
                            showUpDownDialog();
                        } else if (fullExpressionName === "Left Right") {
                            showLeftRightDialog();
                        } else if (fullExpressionName === "Water Float") {
                            showWaterFloatDialog();
                        } else if (fullExpressionName === "Glitter") {
                            showGlitterDialog();
                        } else if (fullExpressionName === "Fish-like") {
                            showFishDialog();
                        } else if (fullExpressionName === "Fast Wiggle") {
                            showWigglePresetsDialog();
                        } else if (fullExpressionName === "Posterize Time") {
                            showPosterizeTimeDialog();
                        } else if (fullExpressionName === "Rotation PingPong") {
                            showRotationPingPongDialog();
                        } else if (fullExpressionName === "Thunder Flicker") {
                            showThunderFlickerDialog();
                        } else if (fullExpressionName === "Scale Pulse") {
                            showScalePulseDialog();
                        } else if (fullExpressionName === "Walk/Run Arc") {
                            showWalkRunDialog();
                        } else {
                            handleExpressionClick(fullExpressionName, expressionCode);
                        }
                    };
                })(key, EXPRESSIONS[key], fullName);
            }
        }
    }
